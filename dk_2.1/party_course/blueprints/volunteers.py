...
def volunteer_overview():
	...
	records_map: dict[int, list[VolunteerRecord]] = defaultdict(list)
	if student_ids:
		for rec in VolunteerRecord.query.filter(
			VolunteerRecord.semester_id == semester.id,
			VolunteerRecord.student_id.in_(student_ids),
		).all():
			records_map[rec.student_id].append(rec)
	...
	for stu in students:
		...
		if period != "全部" and stu_period != period:
			continue
		records = records_map.get(stu.id, [])
