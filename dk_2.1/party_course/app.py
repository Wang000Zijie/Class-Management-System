...
def _ensure_default_accounts() -> None:
    presets = [
        {
            "username": "admin",
            "password": os.environ.get("DEFAULT_ADMIN_PASSWORD") or "admin123",
            "role": "admin",
            "real_name": "系统管理员",
        },
        {
            "username": "buzhang",
            "password": os.environ.get("DEFAULT_MINISTER_PASSWORD") or "buzhang123",
            "role": "minister",
            "real_name": "部长账号",
        },
        {
            "username": "ganshi",
            "password": os.environ.get("DEFAULT_STAFF_PASSWORD") or "ganshi123",
            "role": "staff",
            "real_name": "干事账号",
        },
    ]
...
def create_app() -> Flask:
    ...
    def audit_mutation_requests(response):
        try:
            ...
            for key, value in request.form.items():
                key_lower = key.lower()
                if any(token in key_lower for token in SENSITIVE_FORM_KEYWORDS):
                    form_parts.append(f"{key}=***")
                    continue
                text_value = str(value or "").replace("\n", " ").replace("\r", " ").strip()
                form_parts.append(f"{key}={text_value[:40]}")
    ...
    def handle_csrf_error(_error):
        if request.accept_mimetypes.accept_json and not request.accept_mimetypes.accept_html:
            return jsonify({"ok": False, "msg": "请求校验失败，请刷新页面后重试。"}), 400

        flash("页面已过期或请求校验失败，请刷新页面后重试。", "danger")
        if current_user.is_authenticated:
